...
public class AttendeeLoadMoreListView extends ListView implements View.OnTouchListener, GestureDetector.OnGestureListener, AbsListView.OnScrollListener {
    ...
    public AttendeeLoadMoreListView(Context context, AttributeSet attrs) {
        super(context, attrs);
     ...
     //   Log.e("arg3--->",arg3+"");
        ...
        if((arg2 < 0 && arg3 > 0) || (arg2 <0 && arg3 <0)) {
            if (!isDelectShow && Math.abs(arg2) > Math.abs(arg3)) {
                //加载侧滑出的控件
                swipeList = LayoutInflater.from(getContext()).inflate(R.layout.list_swipe, null);
                ...
                try {
                    int s = onSetStatusListener.setCheck(selectedItem);
                    setViewStatus(s);
