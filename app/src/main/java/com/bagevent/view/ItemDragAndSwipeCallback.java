...
//    private static final String TAG = ItemDragAndSwipeCallback.class.getSimpleName();
    ...
    public void onChildDrawOver(Canvas c, RecyclerView recyclerView, RecyclerView.ViewHolder viewHolder,
        ...
        if (actionState == ItemTouchHelper.ACTION_STATE_SWIPE
            ...
            c.save();
          //  Log.e("height",itemView.getHeight()+"F");
            if(itemView.getHeight() <= 213 || itemView.getHeight() <= 215) {
                if (dX > 0) {
                    c.clipRect(itemView.getLeft(), itemView.getTop(),
                            itemView.getLeft() + dX, itemView.getBottom());
                    c.translate(itemView.getLeft(), itemView.getTop());
                    //  Log.e("tag","left = " + itemView.getLeft() + " dx=" + dX);
                } else {
                    c.clipRect(itemView.getRight() + dX, itemView.getTop(),
                            itemView.getRight(), itemView.getBottom());
                    c.translate(itemView.getRight() + dX, itemView.getTop());
                    //   Log.e("tag","right = " + itemView.getLeft() + " dx=" + dX);

                }
            }else {
                if (dX > 0) {
                    c.clipRect(itemView.getLeft(), itemView.getTop() + 78,
                            itemView.getLeft() + dX, itemView.getBottom());
                    c.translate(itemView.getLeft(), itemView.getTop() + 78);
                    //  Log.e("tag","left = " + itemView.getLeft() + " dx=" + dX);
                } else {
                    c.clipRect(itemView.getRight() + dX, itemView.getTop() + 78,
                            itemView.getRight(), itemView.getBottom());
                    c.translate(itemView.getRight() + dX, itemView.getTop() + 78);
                    //   Log.e("tag","right = " + itemView.getLeft() + " dx=" + dX);
